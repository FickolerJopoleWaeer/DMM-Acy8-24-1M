...
# метод обновления ходов (inc) - прибавить
    def след_ход(self):
        if len(self.Порядок_ходов) == self.Текущий_ход+1: # если номер хода этого был последним (равен кол-ву участников), то сбрасываем ходы
            Data.Room.update_one({"Название": self.Название}, {"$set": {'Текущий_ход': 0}}) # делает переход к первому ходу (0)
        else:
...
# Изменяем поле Колода: уменьшаем при раздаче 
    def обновляем_колоду(self, карты: list): # карты: список карт, которые нужно записать в поле 'Карты_стола'.
        Data.Player.update_one({"Название": self.Название}, {"$set": {"Колода": карты}})
        return self.__init__(self.Название)
...
# Устанавливаем/перезаписываем поле 'Карты_стола' 
    def стол_раздать(self, карты: list): # карты: список карт, которые нужно записать в поле 'Карты_стола'.
        Data.Player.update_one({"Название": self.Название}, {"$set": {"Карты_стола": карты}})
        return self.__init__(self.Название)
...
# Альтернатива записи карт (добавление в существующий список новых, не перезаписывая список.)
    def стол_добавить(self, карты: list): # карты: список карт, которые нужно добавить в поле 'Карты_стола'.
        Data.Player.update_one({"Название": self.Название}, {"$push": {"Карты_стола": {"$each": карты}}})
        return self.__init__(self.Название)
