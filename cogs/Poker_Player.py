...
•	Недостаточно монет, +
    ...
    # Проверка всех ошибок сразу
        ...
        if not await FuncP.check_error_blind(interaction = interaction, room = room, Ставка = Ставка):
            return
...
•	Недостаточно монет, +
    ...
    # Проверка всех ошибок сразу
        ...
        if not await FuncP.check_error_blind(interaction, room = room, Ставка = Ставка):
            return
    # Данные для вывода текста
        Тип_ставок: str = room.Тип_ставок
        Макс_ставка: int = room.Макс_ставка
...
Побеждает <@{IDwin}>, и забирает весь банк ({Выигрыш}) себе!\n(Прибыль: {Выигрыш-Проигрыш})\n\
\nПодсказка:\n\
